 ...
 */
namespace Ecommerce66\Widgets\Block\Products;

use Magento\Catalog\Api\CategoryRepositoryInterface;
use Ecommerce66\Widgets\Block\Proxy\AbstractProduct;
 ...
 */
class BySku extends AbstractProduct implements BlockInterface, IdentityInterface
{
    /**
    ...
    /**
     * Default value for products per row
     */
    const DEFAULT_PRODUCTS_PER_ROW = 2;

    /**
     * Default value for products per row tablet
     */
    const DEFAULT_PRODUCTS_PER_ROW_TABLET = 2;

    /**
     * Default value for products per row mobile
     */
    ...
    */
    public function getCacheKeyInfo()
    {
        ...
        return [
            ...
            $this->getUseSlider(),
            $this->getPerRow(),
            $this->getPerRowTablet(),
            $this->getPerRowMobile(),
            $this->getNumberRow(),
    ...
    public function createCollection()
    {
        ...
        $collection = $this->_addProductAttributesAndPrices($collection)
            ...
            $skus =  explode(',', !empty($skus) ? $skus : '');
            $skus_string = "'" . str_replace(",", "','", $this->getData('skus')) . "'";
            $collection->getSelect()->reset('order');
            $collection->addFieldToFilter('sku', ['in' => $skus]);
            $collection->getSelect()->reset('order')->order($this->resetOrder($skus_string));
     ...
     */
    private function resetOrder($skusOrder)
    {
        return new \Zend_Db_Expr("FIELD(sku,$skusOrder)");
    }
    ...
    {
        if (!$this->hasData('use_slider')) {
            $this->setData('use_slider', 1);
        }
        return $this->getData('use_slider');
    }
    /**
     * Retrieve whishlist show
     *
     * @return int
     */
    public function getHideWishlist()
    {
        if (!$this->hasData('hide_addwishlist')) {
            $this->setData('hide_addwishlist', 0);
        }
        return $this->getData('hide_addwishlist');
    }
    /**
     * Retrieve compare show
     *
     * @return int
     */
    public function getHideCompare()
    {
        if (!$this->hasData('hide_addcompare')) {
            $this->setData('hide_addcompare', 0);
        }
        return $this->getData('hide_addcompare');
    }
    /**
     * Retrieve cart Show
     *
    ...
    {
        if (!$this->hasData('hide_addcart')) {
            $this->setData('hide_addcart', 1);
        }
        return $this->getData('hide_addcart');
    }
    /**
     * Retrieve how many products should be displayed
     *
    ...
    /**
     * Retrieve how many products should be displayed in desktop
     *
     * @return int
     */
    public function getPerRow()
    {
        if (!$this->getData('perrow')) {
            $this->setData('perrow', self::DEFAULT_PRODUCTS_PER_ROW);
        }
        return $this->getData('perrow');
    }

    /**
     * Retrieve how many products should be displayed in tablet
     *
     * @return int
     */
    public function getPerRowTablet()
    {
        if (!$this->getData('perrow_tablet')) {
            $this->setData('perrow_tablet', self::DEFAULT_PRODUCTS_PER_ROW_TABLET);
        }
        return $this->getData('perrow_tablet');
    }
    ...
    {
        if (!$this->getData('number_row')) {
            $this->setData('number_row', self::DEFAULT_PRODUCTS_PER_ROW);
        }
        return $this->getData('number_row');
    }

    /**
     * Retrieve how many products should be displayed in mobile
     *
     * @return int
     */
    public function getPerRowMobile()
    {
        if (!$this->getData('perrow_mobile')) {
            $this->setData('perrow_mobile', self::DEFAULT_PRODUCTS_PER_ROW_MOBILE);
        }
        return $this->getData('perrow_mobile');
    }
