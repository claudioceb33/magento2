...
namespace Ecommerce66\MultilineDiscount\Model;
...
class Validator extends \Magento\SalesRule\Model\Validator
{
    ...
    public function process(AbstractItem $item, Rule $rule)
    {
        ...
        $appliedRuleIds = $this->rulesApplier->applyRules(
            ...
            $this->getCouponCode() ? [$this->getCouponCode()]:[]
        );
    ...
    public function processShippingAmount(Address $address)
    {
        ...
        foreach ($this->getRules($address) as $rule) {
            ...
            switch ($rule->getSimpleAction()) {
                ...
                case Rule::BY_PERCENT_ACTION:
                    ...
                    $this->addAppliedRule($rule, $discountAmount, $appliedRules);
        ...
        $this->checkoutSession->setCustomDiscounts(json_encode($appliedRules));
        $address->setCustomDiscounts(json_encode($appliedRules));
        return $this;
    }
    ...
    public function getRuleItemTotalsInfo($key)
    {
        if (empty($this->_rulesItemTotals[$key])) {
            throw new LocalizedExceptionMagento\Framework\Exception\LocalizedException(__('Item totals are not set for the rule.'));
        }
    ...
    protected function addAppliedRule($rule, $discountAmount, &$appliedRules)
    {
        if (!array_key_exists($rule->getId(), $appliedRules)) {
            $appliedRules[$rule->getId()] = [
                'rule_id' => $rule->getId(),
                'rule_label' => (!empty($rule->getStoreLabels()) && array_key_exists(0, $rule->getStoreLabels()))?$rule->getStoreLabels()[0]:__('Discount'),
                'amount' => 0
            ];
        }
        $appliedRules[$rule->getId()]['amount'] = $appliedRules[$rule->getId()]['amount'] + $discountAmount;
    }
}
